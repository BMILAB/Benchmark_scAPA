...
LOG_FILE="/mnt/T20T-d2bf/BXY/scAPA/sim.data/PAC/tair_GSM3490690/polyapipe/performance_log.txt"
...
do echo $sample;
mkdir /mnt/T20T-d2bf/BXY/scAPA/sim.data/PAC/tair_GSM3490690/polyapipe/${sample};
cd /mnt/T20T-d2bf/BXY/scAPA/sim.data/PAC/tair_GSM3490690/polyapipe/${sample}
...
echo -e "\n[polyApipe.py 执行]" >> "$LOG_FILE"
  ...
  /usr/bin/time -f "峰值内存: %M KB\n平均内存: %K KB\nCPU使用率: %P\n运行时长: %e秒" \
    -a -o "$LOG_FILE" \
/mnt/user/BXY/scAPA/scAPA_tools/polyApipe-master/polyApipe.py -i /mnt/T20T-d2bf/BXY/scAPA/sim.data/SRA/tair_GSM3490690/${sample}.bam -o /mnt/T20T-d2bf/BXY/scAPA/sim.data/PAC/tair_GSM3490690/polyapipe/${sample}/${sample} --t 12 --cell_barcode_tag CB --umi_tag UB
