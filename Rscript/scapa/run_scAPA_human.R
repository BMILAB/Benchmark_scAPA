...
require("Rsubread")
require("scAPA")
source("/mnt/user/BXY/scAPA/code/scAPA/new_fuwuqi2/read_down.seq2.R")
source("/mnt/user/BXY/scAPA/code/scAPA/new_fuwuqi2/intersect_peaks2.R")
...
##species("Hs" for human(h19);"Hg38" for human(h38);"Mm" for mouse(mm10))
org="Hg38"
####fasta files----------------------------------------------
char.length.path="/mnt/user/BXY/ref_file/GRch38fa+gtf/genecode/v34/main.chrNameLength.txt"
fasta.path="/mnt/user/BXY/ref_file/GRch38fa+gtf/genecode/v34/GRCh38.p13.genome.fa"

###3UTR bed file-------------------------------------------------------------------
##When ref genome is hg39,need 3UTR bed file
utr="/mnt/user/BXY/ref_file/GRch38fa+gtf/genecode/v34/scapa_utr3.bed"

###read cell annotation file----------------------------------

samplelist=c("pas1_gn5000_bn3000_rep1","pas1_gn5000_bn3000_rep2","pas1_gn5000_bn3000_rep3")

for(i in 1:length(samplelist)){

samples=samples=samplelist[i]

cell.file=paste0("/mnt/T20T-d2bf/BXY/scAPA/sim.data/SRA/human_GSM4712885")
...
outpath=paste0("/mnt/T20T-d2bf/BXY/scAPA/sim.data/PAC/human_GSM4712885/scapa/",samples)
...
peaks.bed <- intersect_peaks2(org = "Hg38", utr_bed=utr,
  ...
  for (j in 1:length(annotation.files)) {
        ...
        split_bams <- function(x) {
            ...
            FilterBAMbyTag.command <- paste0(
                                             "/mnt/user/BXY/software/Drop-seq_tools-2.2.0/FilterBamByTag ",
                                             "TAG=CB I=",
                                             merged.bam, " O=",
                                             "CellBams/", bam,
                                             " TAG_VALUE=", cell)
